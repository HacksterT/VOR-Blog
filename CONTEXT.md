...
- **Contact pipeline:** all five forms (`/contact`, Selah, am-i-saved, `/ai-ministry`, `/walk`) POST through `src/lib/formSubmit.ts` to the shared Cloudflare forms-worker at `https://forms-worker.troysybert.workers.dev/submit`, which verifies Turnstile, derives `site` from the Origin, writes one row to Cloudflare D1 `forms-db`, and pings Telegram. That Worker lives in `/Users/hackstert/Projects/crm`, which owns the form backend for all three of Troy's sites. **Ezra is out of this path entirely** — sunset, with its `vor_crm.db` migrated to D1 on 2026-07-03. The Worker sends no VOR email, which is why the am-i-saved assessment is currently undelivered (see `tasks/F03-S05-am-i-saved-assessment-email.md`). `/walk` submits `source: 'walk'` with `metadata.path`; beehiiv is intended to be wired **worker-side**, so no API key ever ships to the client.
